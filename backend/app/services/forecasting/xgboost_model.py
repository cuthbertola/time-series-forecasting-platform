...
class XGBoostForecaster(BaseForecaster):
    ...
    def predict_interval(
        ...
        """Generate predictions with confidence intervals using residual-based approach."""
    ...
    def predict_quantiles(
        ...
        """Generate quantile predictions using bootstrapping."""
        if not self.is_fitted:
            raise ValueError("Model must be fitted before prediction")
        
        predictions = self.predict(X)
        
        # Use residual-based quantile estimation
        from scipy import stats
        
        results = {}
        for q in quantiles:
            z = stats.norm.ppf(q)
            results[q] = predictions + z * self.residual_std
